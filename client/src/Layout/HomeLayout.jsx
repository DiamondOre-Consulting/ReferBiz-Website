...
import { FaCar, FaHotel, FaArrowDown, FaArrowRight, FaRegUser, FaUser } from "react-icons/fa"
import { HiOutlineXMark } from "react-icons/hi2"
import { CgLogOut } from "react-icons/cg"
import { MdContentPaste, MdOutlineSettings, MdOutlineDashboard, MdOutlineContactMail } from 'react-icons/md'
import { RxHamburgerMenu } from "react-icons/rx"
import { RiFileList2Fill, RiGalleryFill, RiUserLocationFill } from "react-icons/ri"
...
const HomeLayout = ({ children }) => {
    ...
    const listStyle = 'flex items-center justify-start gap-2 pl-4 p-2 m-[0.4rem] ml-0 mr-3 md:mr-4 rounded-r bg-[#2A303D] hover:bg-[#6761D9] hover:text-white transition-all duration-300 text-[#CBC8E0] font-semibold tracking-wide text-[1.02rem]'
                ...
                <div className='w-full'>
                    <header className='flex items-center justify-between backdrop-blur-3xl relative z-[1000000] px-3 bg-[#212631] p-2 md:w-full text-[#fff] border-b border-[#323A49]'>
                        ...
                        <div className='flex items-center justify-center gap-5 pr-1'>
                            ...
                            <Link to={`/profile/${fullName}`} className=' size-[2.6rem] rounded-full overflow-hidden bg-[#a4a5a9] border border-borderDark'>
                                <img src={avatar?.secure_url || "userImg"} className='w-[2.55rem]' alt="User Avatar" />
                            </Link>
                        </div>
                    </header>
                ...
                <NavLink className={`z-[100000000] h-[100vh] border-r border-borderDark overflow-hidden max-w-[15rem] min-w-[15rem] md:max-w-[16rem] md:min-w-[15.9rem]  bg-[#212631] absolute md:static top-0 ${active ? 'left-0' : 'left-[-35rem]'} transition-all duration-500`}>
                    <ul>
                        ...
                        <div className='h-[82vh] overflow-y-scroll scrollbar scrollbar-none'>
                            ...
                            <li>
                                <NavLink to={'/vendor/global-settings'} className={({ isActive }) => isActive ? activeListStyle : listStyle}>
                                    <RiFileList2Fill />
                                    Orders
                                </NavLink>
                            </li>
                    ...
                    <Link onClick={handleLogout} className='bg-[#DD4141] transition-all duration-700 hover:bg-[#c22f2f] border-none text-white flex items-center gap-2 pl-4 p-2 m-[0.4rem] ml-0 mr-3 rounded-r md:mr-4 font-semibold text-[1.02rem] tracking-wide'>
